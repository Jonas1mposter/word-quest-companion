...
import { ChevronLeft, Volume2, Check, X, Star, RotateCcw, GraduationCap } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "sonner";
import { useSpeech } from "@/hooks/useSpeech";
import type { HSWord } from "./HSLevelProgress";

interface HSWordLearningProps {
  levelId: string;
  levelName: string;
  words: HSWord[];
...
const HSWordLearning = ({ levelId, levelName, words, startPhase = "learn", onBack, onComplete }: HSWordLearningProps) => {
  ...
  const generateOptions = useCallback((correctWord: HSWord) => {
    const options = [correctWord.meaning];
    const others = words.filter(w => w.id !== correctWord.id);
    while (options.length < 4 && others.length > 0) {
      const idx = Math.floor(Math.random() * others.length);
      const w = others.splice(idx, 1)[0];
  ...
  const finishLevel = async (finalCorrect: number) => {
    ...
    setXpEarned(5 + Math.floor(accuracy * 5));
    setCoinsEarned(2 + (accuracy === 1 ? 3 : Math.floor(accuracy * 2)));
    setPhase("result");
    ...
    if (profile) {
      try {
        ...
        await supabase.functions.invoke("increment-quest-progress", { body: { questType: "learn", amount: 1 } });
        if (finalCorrect > 0) {
          await supabase.functions.invoke("increment-quest-progress", { body: { questType: "words", amount: finalCorrect } });
        }
        await refreshProfile();
        queryClient.invalidateQueries({ queryKey: ["hs-learning-progress", profile.id] });
      } catch (err) {
      ...
      try {
        ...
        if (existing) {
          await supabase
            .from("hs_learning_progress" as any)
            .update({
              mastery_level: Math.max((existing as any).mastery_level ?? 0, isCorrect ? 1 : 0),
              correct_count: ((existing as any).correct_count ?? 0) + (isCorrect ? 1 : 0),
              incorrect_count: ((existing as any).incorrect_count ?? 0) + (isCorrect ? 0 : 1),
              last_reviewed_at: nowIso,
              updated_at: nowIso,
            })
            .eq("id", (existing as any).id);
        } else {
          await supabase.from("hs_learning_progress" as any).insert({
        ...
        <div className="max-w-lg mx-auto space-y-4">
          ...
          <Card className="border-2 border-amber-500/30">
            <CardContent className="p-6 text-center space-y-4">
              ...
              <Badge variant="outline" className="text-xs">{currentWord?.unit_name || `Unit ${currentWord?.unit}`}</Badge>

              {showMeaning ? (
                <div className="space-y-3 animate-fade-in">
                  <p className="text-xl text-amber-500 font-medium">{currentWord?.meaning}</p>
                ...
                <Button variant="outline" onClick={() => setShowMeaning(true)} className="w-full">
                  显示释义
                </Button>
              )}
            </CardContent>
          ...
          {showMeaning && (
            <Button variant="hero" className="w-full" onClick={handleLearnNext}>
            ...
            {words.map((word, idx) => (
              <div
                ...
                className={cn(
                  ...
                  idx === currentIndex && "w-4 bg-amber-500",
              ...
              return (
                <Button
                  ...
                  className={cn(
                    ...
                    !isAnswered && "hover:bg-amber-500/10 hover:border-amber-500"
                  )}
    ...
    <div className="min-h-screen bg-background bg-grid-pattern p-4 flex items-center justify-center">
      <Card className="w-full max-w-md border-2 border-amber-500/30">
        <CardContent className="p-6 text-center space-y-6">
          <div className="space-y-2">
            <GraduationCap className="w-12 h-12 mx-auto text-amber-500" />
            ...
            {[1, 2, 3].map((star) => (
              <Star
                ...
                className={cn(
                  ...
                  star <= stars ? "text-accent fill-accent animate-bounce-in" : "text-muted-foreground/30"
                )}
