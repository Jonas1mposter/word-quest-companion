...
BEGIN
  ...
  -- 清理调用者残留的队列条目：
  -- 1) 已结束/已完成对局对应的 matched 行（房间已不再有效）
  -- 2) 超过 10 分钟仍处于 matched 状态的僵尸行
  -- 3) 自己之前的 searching 行（避免与新查询冲突）
  ...
  WHERE mq.profile_id = _profile_id
    AND (
      ...
      OR (
        mq.status = 'matched'
        AND (
          mq.created_at < now() - interval '10 minutes'
          OR EXISTS (
  ...
  -- 同样跳过对手那侧已绑定到「已完成」对局的僵尸 matched 行
